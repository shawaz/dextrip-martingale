...
try:
    ...
    temp_client = ClobClient(
        HOST, key=PRIVATE_KEY, chain_id=CHAIN_ID, signature_type=2, funder=FUNDER
    )
...
def get_market_tokens(slug):
    if slug in MARKET_TOKEN_CACHE:
        return MARKET_TOKEN_CACHE[slug]
    try:
...
def place_buy(token_id, amount):
    try:
        order = MarketOrderArgs(
            token_id=token_id, amount=round(float(amount), 2), side=BUY
        )
...
def run_executor():
    ...
    while True:
        try:
            ...
            print(
                f"[STATUS] Round: {round_id} | T-minus: {int(seconds_to_start)}s | Active Signals: {len(recommended)}",
                end="\r",
            )
